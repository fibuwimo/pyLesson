import tkinter as tk

index=0
tsugi=1
timer=0
count=0

cursor_x=0
cursor_y=0
mouse_x=0
mouse_y=0
mouse_c=0

def mouse_move(e):
    global mouse_x, mouse_y
    mouse_x = e.x
    mouse_y = e.y

def mouse_press(e):
    global mouse_c
    mouse_c = 1


neko = []
check = []
for i in range(10):
    neko.append([0,0,0,0,0,0,0,0])
    check.append([0,0,0,0,0,0,0,0])


def draw_neko():
    cvs.delete("NEKO")
    for y in range(10):
        for x in range(8):
            if neko[y][x]>0:
                cvs.create_image(x*72+60,y*72+60,image=img_neko[neko[y][x]],tag="NEKO")

def check_neko():
    for y in range(10):
        for x in range(8):
            check[y][x]=neko[y][x]

    for y in range(1,9):
        for x in range(8):
            if check[y][x] > 0:
                if check[y-1][x] == check[y][x] and check[y+1][x]==check[y][x]:
                    neko[y-1][x]=7
                    neko[y][x]=7
                    neko[y+1][x]=7

    for y in range(10):
        for x in range(1,7):
            if check[y][x] > 0:
                if check[y][x-1] == check[y][x] and check[y][x+1]==check[y][x]:
                    neko[y][x-1]=7
                    neko[y][x]=7
                    neko[y][x+1]=7

    for y in range(1,9):
        for x in range(1,7):
            if check[y][x] > 0:
                if check[y-1][x-1] == check[y][x] and check[y+1][x+1]==check[y][x]:
                    neko[y-1][x-1]=7
                    neko[y][x]=7
                    neko[y+1][x+1]=7
                if check[y+1][x-1] == check[y][x] and check[y-1][x+1]==check[y][x]:
                    neko[y+1][x-1]=7
                    neko[y][x]=7
                    neko[y-1][x+1]=7

def sweep_neko():
    issweep=False
    for y in range(10):
        for x in range(8):
            if neko[y][x]==7:
                neko[y][x]=0
                issweep=True
    return issweep 

def draw_txt(txt,x,y,siz,col,tg):
    fnt=("Times New Roman",siz,"bold")
    cvs.create_text(x+2,y+2,text=txt,fill="black",font=fnt,tag=tg)
    cvs.create_text(x,y,text=txt,fill=col,font=fnt,tag=tg)

def game_main():
    global cursor_x,cursor_y,mouse_c
    global index,timer,tsugi,count

    if index ==0:
        draw_txt("三目並べ",312,240,100,"violet","TITLE")
        cvs.create_rectangle(168,384,456,456,fill="skyblue",width=0,tag="TITLE")
        draw_txt("開始",312,420,40,"white","TITLE")
        index=1
        tsugi=1
        mouse_c=0
    elif index==1:
        if mouse_c==1:
            if 168<mouse_x and mouse_x <456 and 384<mouse_y and mouse_y <456:
                for y in range(10):
                    for x in range(8):
                        neko[y][x]=0
                mouse_c=0
                tsugi=0
                cursor_x=0
                cursor_y=0
                draw_neko()
                cvs.delete("TITLE")
                index=3
    elif index==3:
        check_neko()
        draw_neko()
        index=4
    elif index==4:
        issweep=sweep_neko()
        if issweep==False:
            if tsugi==1:
                tsugi=2
            else:
                tsugi=1
            index=5
        else:
            index=6
            timer=0
        if count==9:
            index=6
            timer=0
    elif index ==5:
        if 24 <= mouse_x and mouse_x < 24+72*3 and 24 <= mouse_y and mouse_y < 24+72*3:
            cursor_x =int((mouse_x-24)/72)
            cursor_y =int((mouse_y-24)/72)
            if mouse_c ==1:
                mouse_c =0
                if neko[cursor_y][cursor_x]==0:
                    neko[cursor_y][cursor_x] = tsugi
                    count+=1
                    index=3
        cvs.delete("CURSOR")
        cvs.create_image(cursor_x*72+60,cursor_y*72+60,image=cursor,tag="CURSOR")
        draw_neko()
    elif index ==6:
        timer=timer+1
        if timer==1:
            if tsugi == 1:
                draw_txt("1p WIN",312,348,60,"red","OVER")
            else:
                draw_txt("2p WIN",312,348,60,"red","OVER")
            if count ==9:
                cvs.delete("OVER")
                draw_txt("drow",312,348,60,"red","OVER")
        if timer ==50:
            cvs.delete("OVER")
            index=0
            count=0
    cvs.delete("INFO")
    if tsugi>0:
        cvs.create_image(752,128,image=img_neko[tsugi],tag="INFO")
    root.after(100,game_main)
        

root=tk.Tk()
root.title("ねこねこねこね")
root.resizable(False,False)
root.bind("<Motion>",mouse_move)
root.bind("<ButtonPress>",mouse_press)
cvs=tk.Canvas(root,width=912,height=768)
cvs.pack()

bg=tk.PhotoImage(file="neko_bg.png")
cursor=tk.PhotoImage(file="neko_cursor.png")
img_neko=[
        None,
        tk.PhotoImage(file="neko1.png"),
        tk.PhotoImage(file="neko2.png"),
        tk.PhotoImage(file="neko3.png"),
        tk.PhotoImage(file="neko4.png"),
        tk.PhotoImage(file="neko5.png"),
        tk.PhotoImage(file="neko6.png"),
        tk.PhotoImage(file="neko_niku.png"),
    ]

cvs.create_image(456,384,image=bg)
game_main()
root.mainloop()
